"""
worker.py — Kafka consumer/producer worker for the C/C++ sandbox pool (beta).

Consumes jobs from `visualize-jobs` (filtered by language == "cpp"),
compiles + executes the submitted code inside a resource-limited subprocess
pipeline (g++ -> binary), and publishes the resulting steps[] trace to
`visualize-results`.
"""
...
GROUP_ID = os.environ.get("KAFKA_GROUP_ID", "sandbox-cpp")
LANGUAGE_FILTER = os.environ.get("LANGUAGE_FILTER", "cpp")
MAX_EXEC_SECONDS = float(os.environ.get("MAX_EXEC_SECONDS", "20"))
MAX_COMPILE_SECONDS = float(os.environ.get("MAX_COMPILE_SECONDS", "15"))
...
def execute_code(code: str) -> dict:
    ...
    tmp_dir = tempfile.mkdtemp(prefix="algo-cpp-job-")
    try:
        ...
        try:
            compile_proc = subprocess.run(
                ["g++", "-std=c++20", "-O1", "-o", bin_path, src_path],
                capture_output=True,
                text=True,
        ...
        if run_proc.returncode != 0:
            ...
            msg = tail[-1] if tail else f"process exited with code {run_proc.returncode}"
...
def main():
    print(f"[sandbox-cpp] starting worker, group={GROUP_ID}, filter={LANGUAGE_FILTER}", flush=True)
    ...
    print("[sandbox-cpp] ready, consuming jobs...", flush=True)
    ...
    for message in consumer:
        ...
        job_id = job.get("id")
        code = job.get("code", "")
        print(f"[sandbox-cpp] processing job {job_id}", flush=True)
        ...
        producer.flush()
        print(f"[sandbox-cpp] published result for {job_id} ({len(result['steps'])} steps)", flush=True)
